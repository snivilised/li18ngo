...
// it is not important for these tests to pass, because storage is
// eventually going to be removed entirely. We already know that the
// functionality works on past performance
...
var _ = Describe("virtual-fs", Ordered, func() {
	...
	BeforeAll(func() {
		root = helpers.Repo("")
		requiem = helpers.Path(root, "Test/data/storage/Nephilim/Mourning Sun/info.requiem.txt")
	})
